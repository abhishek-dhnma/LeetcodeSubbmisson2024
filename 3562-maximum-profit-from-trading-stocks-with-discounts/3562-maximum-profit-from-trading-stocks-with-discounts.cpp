...
public:
    void DFS(int u, const vector<int>& present, const vector<int>& future,
             ...
             vector<vector<vector<int>>>& statesProfit, int budget) {
        ...
        for (int v : adj[u]) {
            ...
            childrenStatesProfit.push_back(
                {statesProfit[v][0], statesProfit[v][1]});
        }
        ...
        for (int parentBought = 0; parentBought <= 1; parentBought++) {
            ...
            for (auto& child : childrenStatesProfit) {
                ...
                for (int used = 0; used <= budget; used++) {
                    for (int take = 0; used + take <= budget; take++) {
                        temp[used + take] =
                            max(temp[used + take],
                                childrenProfitIfUNotBought[used] +
                                    child.first[take]);
                    }
                }
            ...
            for (int b = 0; b <= budget; b++) {
                ...
                bestProfitAtU[b] =
                    max(bestProfitAtU[b], childrenProfitIfUNotBought[b]);
            }
